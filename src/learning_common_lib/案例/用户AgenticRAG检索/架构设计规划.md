...
12. **首版必须覆盖等待与澄清状态**。除了 `EXECUTING`，还需要明确 `WAITING_SUBTASKS`、`WAITING_CLARIFICATION`、`FINALIZING` 等过渡态；Clarify 首版限制为最多 1 次、单选题形式、超时自动采用默认选项。
...
flowchart TB
    ...
    StepGate -- "需要用户补充" --> WaitClarify
    WaitClarify -- "收到补充信息" --> Planner
...
> 3. GlobalGraph 在分发后进入 `WAITING_SUBTASKS`，直到 `resume_orchestrator` 恢复。
> 4. `resume_orchestrator` 只做结果验收、恢复和幂等防重，不单独承担 READY 判定与全局推进决策。
> 5. Worker 完成后先同步提交控制面状态，再把证据卡、评估记录、草稿输出等数据面 payload 交给 `persist_jobs` 异步刷入 MySQL。
...
flowchart TD
    ...
    Router{"路由决策<br/>是否需要检索 / 直接消费全局证据"}
    CacheProbe{"语义缓存探测"}
    ...
    CacheProbe -- "未命中且需要检索" --> RewriteRoute
    CacheProbe -- "未命中但已有全局证据可复用" --> EvalScore
...
> 3. `Clarify` 不会直接推进主计划，而是把全局任务切到 `WAITING_CLARIFICATION`，等待用户补充后恢复。
...
flowchart LR
    ...
    Planner -- "读 L3-Control / L3-Evidence" --> L3C
    Planner -- "读证据摘要" --> L3E
    Planner -- "写 DAG / waiting_reason" --> L3C
    Orchestrator -- "读任务状态" --> L3C
    Retrieval -- "读/写 L2" --> L2
    Retrieval -- "写证据卡到 L3-Evidence" --> L3E
    Judge -- "读 L2 + L3-Evidence" --> L2
    Judge -- "读 L2 + L3-Evidence" --> L3E
    Verifier -- "读 L2" --> L2
    Assembler -- "读 L3-Control / L3-Evidence" --> L3C
...
> **说明**：L3 明确拆成控制子域和证据子域。控制子域必须同步写 MySQL；证据子域优先写 Redis 热池与 staging，再异步归档。L5 继续保留为长期记忆层，但首版只要求接口和数据契约预留，不要求与主调度链路同步落地。
...
| Clarification Agent | 生成追问并接收补充 | `search_tasks`、`sessions` | `search_tasks.status/control_json`、`session_turns`、`task_events` | `ClarificationRequest` |
...
sequenceDiagram
    ...
    CW->>RO: emit SubtaskResultEnvelope
...
- `control_json` json — 控制信号（policy/threshold_profile/stop_reason/waiting_reason）
...
- `selected_option_id` varchar(64) — Clarify 场景下用户选择的选项
...
- `WAITING_CLARIFICATION`：系统已生成追问，等待用户补充关键信息
...
> **边界约束**：
>
> 1. `resume_orchestrator` 只负责写回结果、追加 `task_events`、恢复 GlobalGraph，不单独做 READY 判定。
...
flowchart TD
    ...
    LoadSpec --> CacheProbe{"语义缓存探测"}
    CacheProbe -- "命中" --> FreshnessCheck{"缓存时效校验<br/>TTL / 数据新鲜度"}
    FreshnessCheck -- "新鲜" --> InjectEvidence["注入缓存证据卡"]
    FreshnessCheck -- "过期" --> NeedRetrieve{"当前模板是否仍需检索"}
    CacheProbe -- "未命中" --> NeedRetrieve
    NeedRetrieve -- "否，直接复用全局证据" --> EvalScore
    NeedRetrieve -- "是" --> RewriteRoute["改写路由决策"]
    ...
    Clarify --> WaitClarify["写入 WAITING_CLARIFICATION<br/>等待用户补充"]
...
1. 每个 `search_task` 最多触发 1 次 Clarify，由 `search_tasks.clarification_count` 计数。
2. Clarify 只允许单选题，不允许开放式追问。选项必须覆盖系统可接受的默认推断路径。
3. 追问只允许落在 4 类关键缺口：时间范围、对象范围、指标口径、比较基线。
4. Clarify 请求写入 `control_json.clarification_request`，同时追加 `task_events.clarification_requested`。
5. 用户在截止时间前未回复时，系统自动采用 `default_option_id`，追加 `clarification_default_applied` 事件，并恢复到 `PLANNING`。
...
3. Worker 追加 `task_events`，向 SSE 通道广播 `subtask.completed` 或 `task.waiting_clarification` 事件，并投递 `persist_jobs` 与轻量 `resume_orchestrator(SubtaskResultEnvelope)`。
4. `persist_jobs` 负责把 `evidence_cards`、`evidence_evaluations`、`subtask_outputs`、L4 摘要等数据面数据异步刷入 MySQL。
5. `resume_orchestrator` 只做结果验收、恢复 `GlobalGraph` 和幂等防重；真正的下一步动作仍由恢复后的 `step_gate` / `scheduler` 决定。
...
1. **GlobalGraph**：负责 Intake → Planner → Scheduler → Executor → StepGate → Replan / Finalize 的主流程。关键节点做 READY 计算、预算预留、claim、恢复判定和最终汇总。
...
4. **关键边界**：首版不依赖 `Send()` 的“自动汇聚到下一个节点”语义来承载异步 Celery 结果；真正的恢复入口只有 `resume_orchestrator`。
...
│   ├── run_service.py           # subtask_runs claim / fencing / resume
│   ├── subtask_service.py       # 子任务执行闭环
│   ├── evidence_service.py      # 证据管理与共享
│   ├── progress_service.py      # SSE 事件写入与回放
│   ├── persist_service.py       # 数据面 staging / flush / reconcile
│   ├── rewrite_service.py       # 查询改写
│   ├── retrieval_service.py     # 多路检索执行
│   ├── verify_service.py        # 输出三重校验
│   ├── assembler_service.py     # Final Assembler
...
│       └── mock_web_search.py
├── workers/
│   ├── orchestrator_worker.py   # resume_orchestrator / replan
│   ├── subtask_worker.py        # 单个子任务闭环执行
│   ├── persist_worker.py        # evidence / output / eval flush
│   └── maintenance_worker.py    # reaper / reconcile / cleanup
├── interface/
│   ├── api.py               # FastAPI 路由
...
`POST /api/v1/search/{request_id}/clarification` 首版只接受单选答案：
...
4. 完成 `WAITING_SUBTASKS` / `WAITING_CLARIFICATION` / `FINALIZING` 状态切换
5. 完成 SSE 事件写入、heartbeat 与 `Last-Event-ID` 回放
6. 完成 GlobalReplan + 循环检测
7. 完成 LoopGuard + Fallback 降级

交付标准：给定一个查询，能生成 DAG、完成 READY 任务抢占，并在 Celery 回写结果后恢复编排继续推进，不出现第二套调度链路。

测试要求：DAG 调度单测（拓扑排序正确性、入度计算、并发度控制、死锁检测、旧 `plan_version` 结果 fencing、`resume_orchestrator` 仅恢复不调度）；Replan 循环检测单测（指纹匹配、相似度阈值、边际收益递减）；SSE 集成测试（首条事件 < 1s、heartbeat、断线续传）。
...
4. 首版 Clarify 最多触发 1 次，且固定为单选题形式；超时未答时自动采用默认选项。
...
13. 首版 Clarify 采用“挂起原任务并等待补充”的模型，不创建新的 `search_task`。
