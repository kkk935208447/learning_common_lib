...
4. **Redis 作为热缓存、checkpoint 与数据面 staging 层**。Redis 负责 LangGraph checkpoint、入度/预算热数据、L2 工作记忆、L3-Evidence/L4 热缓存、SSE 实时进度扇出、短 TTL 锁和数据面暂存；Redis 故障时控制面必须能完全回退到 MySQL 重建。
...
> 5. Worker 完成后，`resume_orchestrator` 触发一次新的 `await GlobalGraph.ainvoke(None, thread_id)` 恢复编排；这是一条新的恢复调用，不是旧协程被图内唤醒。
