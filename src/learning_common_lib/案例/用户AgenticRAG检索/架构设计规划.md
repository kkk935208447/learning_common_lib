...
def reap_stuck_tasks():
    ...
    for st in stuck:
        ...
        # 释放 Redis 执行锁
        redis.delete(f"rag:search:subtask:{st.task_id}:{st.subtask_code}")
        # 触发 Scheduler 重新评估
        trigger_scheduler_check.apply_async(args=[st.task_id])
```
...
```python
def rebuild_redis_hot_cache(task_id: int):
    ...
    # 重建全局任务状态
    redis.hset(f"rag:search:task:{task_id}:state", mapping={...})
    # 重建 DAG 入度表
    rebuild_indegree_from_mysql(task_id, task.active_plan_version)
    # 重建证据池
    rebuild_evidence_pool_from_mysql(task_id)
```
...
```python
REPLAN_LOCK_KEY = "rag:search:replan:{task_id}"
REPLAN_LOCK_TTL = 30  # 秒

def try_replan(task_id: int, reason: str) -> bool:
    """尝试获取 Replan 锁，保证互斥"""
    acquired = redis.set(REPLAN_LOCK_KEY.format(task_id=task_id),
                         value=reason, nx=True, ex=REPLAN_LOCK_TTL)
    if not acquired:
        logger.info(f"Replan already in progress for task {task_id}, skipping")
        return False
    try:
        do_replan(task_id, reason)
    finally:
        redis.delete(REPLAN_LOCK_KEY.format(task_id=task_id))
    return True
```
...
> 首版通过 WebSocket 推送进度，Webhook 作为后续扩展预留接口定义。
...
测试要求：DAG 调度单测（拓扑排序正确性、入度计算、并发度控制、死锁检测）；Replan 循环检测单测（指纹匹配、相似度阈值、边际收益递减）。
...
测试要求：端到端测试（从 API 发起请求到返回结果的完整链路）；负载测试基线（使用 locust 或 k6，目标 QPS 50，P95 延迟 < 5s，记录基线数据供后续对比）；故障注入测试（模拟 Redis 宕机、LLM 超时、Worker 崩溃）。
...
8. 并行度限制由 `scheduler_node` 中 `ready[:MAX_CONCURRENT]` 切片实现（参见 [技术拆解 Section 2.7](./技术拆解.md#langgraph-dag)）。
