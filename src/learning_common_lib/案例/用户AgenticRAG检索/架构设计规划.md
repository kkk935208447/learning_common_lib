...
3. **控制面以 MySQL 为真理源，数据面按层分级持久化**。DAG 版本、子任务状态、执行尝试、预算流水、审计事件、Clarify 元数据等控制面数据必须同步提交 MySQL；`evidence_cards`、`evidence_evaluations`、`subtask_outputs`、L4 会话摘要等数据面数据允许先写 Redis staging，再通过异步任务刷入 MySQL。对于非金融场景，允许极少量数据面 staging 丢失，但不允许控制面丢失。
4. **Redis 作为热缓存、checkpoint 与数据面 staging 层**。Redis 负责 LangGraph checkpoint、入度/预算热数据、L2 工作记忆、L3-Evidence/L4 热缓存、SSE 实时进度扇出、短 TTL 锁和数据面暂存、query 语义缓存；Redis 故障时控制面必须能完全回退到 MySQL 重建。
...
方案A：单 Worker 跑完整个流程:
  FastAPI → TaskIQ → Worker-A
                     └── GlobalGraph
                         └── Send() 并行
                             ├── SubtaskGraph-1  ← asyncio 协程
                             ├── SubtaskGraph-2  ← asyncio 协程
                             └── SubtaskGraph-3  ← asyncio 协程
  优点：
    架构极简，没有跨进程协调
    没有 interrupt/Command 的复杂性
    没有 Done redis stream / 回调任务
    调试直接，日志集中在一个进程
    LangGraph Send() 原生并发，延迟最低
  缺点：
    单 Worker 进程崩溃 = 整个任务丢失
    SubtaskGraph 里的 CPU 密集操作阻塞整个 event loop
    无法跨机器分配子任务
    任务时间长（30-60s），Worker 进程长期占用


方案B：总图 + 子图拆开（本文的架构）
  FastAPI   ──→   orchestrator_queue   ──→   Worker-A（GlobalGraph）
                                              └── dispatch
  execute_queue   ──→   Worker-B（SubtaskGraph-1）
  execute_queue   ──→   Worker-C（SubtaskGraph-2）
  execute_queue   ──→   Worker-D（SubtaskGraph-3）
  优点：
    进程级隔离，单个子任务崩溃不影响其他
    子任务可横向扩展到多台机器
    Worker 资源按类型精细分配
  缺点：
    跨进程协调复杂（interrupt/Command/回调）
    消息 round-trip 增加延迟
    调试困难，日志分散
...
flowchart TB
    ...
    subgraph GlobalLoop["全局循环 - orchestrate_jobs Worker 内的 LangGraph GlobalGraph"]
...
> 3. FastAPI 只负责创建任务、查询状态、SSE 推送和 Clarify 提交；`GlobalGraph` 主体运行在 `orchestrate_jobs` worker（或等价的编排 worker）中，而不是挂在 FastAPI 请求协程里等待完成。
> 4. GlobalGraph 在分发后进入 `WAITING_SUBTASKS`；如果编排实现为 async，当前这次 `await GlobalGraph.ainvoke(...)` 会到此结束，不保留协程去等待 Worker 结果。
> 5. Worker 完成后，`resume_orchestrator` 触发一次新的 `await GlobalGraph.ainvoke(None, thread_id)` 或 `await GlobalGraph.ainvoke command` 恢复编排；这是一条新的恢复调用，不是旧协程被图内唤醒。
> 6. `resume_orchestrator` 只做结果验收、恢复和幂等防重，不单独承担 READY 判定与全局推进决策。
> 7. Worker 完成后先同步提交控制面状态，再把证据卡、评估记录、草稿输出等数据面 payload 交给 `persist_jobs` 异步刷入 MySQL。
> 8. Clarify 只能由 `Planner` 或 `StepGate` 触发；子任务只能上报 `needs_user_input` 升级信号，不能直接进入 `WAITING_CLARIFICATION`。
> 9. `clarification_source=PREPLAN` 时，回复回到 `Planner`；`clarification_source=STEP_GATE` 时，回复回到 `StepGate`，不强制重新规划。
> 10. 当前主线不是“FastAPI 挂着等待整个 GlobalGraph 完成”，也不是“单个 worker 在一个事件循环里用 `Send()` 把所有 SubtaskGraph 当 asyncio 协程跑完”；而是“编排 worker 分段执行 GlobalGraph + 执行 worker 独立跑 SubtaskGraph + `resume_orchestrator` 触发下一次恢复”。
> 11. 文中若出现同步 `invoke(...)` 伪代码，只代表外层同步封装；推荐主路径统一采用 `ainvoke(...)`。`asyncio.to_thread(...)` 只允许用于 async producer 发布 Celery 同步 API，不允许用于 `...get()` 等结果等待。
...
flowchart TD
    ...
    subgraph RetrievalFlow["检索与证据构建"]
        ...
        Merge["证据汇聚 + 去重"]
        Fusion["RRF 排序融合"]
        Rerank["轻量重排 → 深度重排(可配合高斯衰减)"]
        PIIRedact["PII 检测与脱敏(进入大模型前)"]
        Compress["证据压缩与原子化"]
        EvalScore["三维评估打分<br/>Coverage(覆盖) / Conflict(冲突) / Confidence(可信度)"]
        Threshold{"阈值判定"}
    end

    subgraph AnswerFlow["答案生成与校验"]
        ParentHydration["父文档按需召回"]
        DraftClaims["生成候选答案<br/>DraftClaims"]
        Verify{"三重校验"}
        CacheWrite["缓存回写"]
        Done["子任务完成"]
    ...
    Retrieval --> Merge --> Fusion --> Rerank --> PIIRedact --> Compress --> EvalScore
    EvalScore --> Threshold
...
> 4. `Verify` 失败后的重试默认只回到 `DraftClaims`(重新生成候选答案) 支路；不重新触发检索，除非缺口诊断明确要求回到检索链路。
> 5. 检索结果在进入 `Compress` 和 L3 全局证据池之前，必须先经过 `PII 检测与脱敏`；未脱敏的原始敏感片段不能直接作为跨子任务共享证据。
...
| Retrieval Agent | 改写、检索、PII 脱敏、证据压缩 | `subtasks`、L2、L3 证据池 | `evidence_cards`、L2 工作记忆 | `evidence_refs` |
...
- `claim` text not null — 脱敏后的共享原子事实断言
...
- `pii_flags_json` json — PII 命中类别、位置与处置摘要
- `redaction_status` enum(`NONE`, `MASKED`, `SUMMARY_ONLY`, `BLOCKED`) not null default `NONE`
- `acl_scope_json` json — 允许访问该证据的角色或策略摘要
...
> **说明**：
> 1. `claim` 默认保存脱敏后的共享文本，不直接把原始敏感片段放进跨子任务共享证据池。
> 2. `redaction_status=BLOCKED` 的证据只保留受控引用或摘要，不进入 L3 全局证据池。
> 3. `consumed_by_json` 不建议继续保留为高频写入字段。消费关系单独落 `evidence_consumptions`，避免多子任务并发消费时反复覆盖 JSON。
...
flowchart TD
    ...
    RewriteRoute --> QV & QK & QS & QG & QW
    QV & QK & QS & QG & QW --> Retrieval
    Retrieval --> Merge --> Fusion --> Rerank --> PIISanitize --> Compress
    Compress --> EvalScore
    InjectEvidence --> EvalScore
    EvalScore --> Threshold
...
3. **PII 检测与脱敏**：在 `Rerank` 之后、进入共享证据链路之前，使用规则引擎 + NER/LLM 辅助识别 + ACL 回查，对手机号、邮箱、身份证、住址、客户唯一标识等敏感内容做 `MASKED`、`SUMMARY_ONLY` 或 `BLOCKED` 处置
4. **原子化**：只对通过脱敏检查的证据执行原子化；LLM 将每条证据拆分为不可再分的原子事实（Atomic Claim），每个 claim 只表达一个独立可验证的事实断言
5. **结构化**：封装为标准证据卡 Schema，附带元数据（来源、可靠性层级、时效性、重排分数、敏感等级、脱敏状态）

补充约束：

1. `PII 检测与脱敏` 发生在共享证据池写入之前，而不是等到最终回答阶段才补救。
2. `redaction_status=BLOCKED` 的证据不进入 L3 全局证据池，只能保留受控引用用于审计或高权限场景。
3. `SUMMARY_ONLY` 证据允许进入证据池，但只以去标识化摘要参与后续推理，不回传原始文本。
...
| L2 | 工作记忆 | 当前子任务的 scratchpad、评估历史、纠偏上下文、子图 checkpoint | 内存 + Redis checkpoint/staging | 子任务运行期；允许宕机丢失 |
| L3-Control | 任务控制状态 | DAG 状态、等待原因、预算、推进信号 | MySQL 真理源 + Redis (SQLalchemy cache-after-commit) | 任务完成后归档 |
| L3-Evidence | 全局证据热池 | 可跨子任务复用的证据卡摘要、已验收子任务产物索引 | Redis hot pool/staging + MySQL archive | 任务完成后归档 |
| L4 | 会话记忆 | 本次对话的交互轨迹、已完成子任务摘要 | Redis hot summary + MySQL archive | 会话期间热缓存，会话后归档 |
| L5 | 长期记忆 | 用户画像、查询模式、语义缓存 | MySQL / 向量库主存 + Redis 热缓存 | 长期保存 / TTL 淘汰 |
...
> **首版边界**：L1 只做“现组装、现消费”，不参与持久化；L5（用户偏好、语义缓存）当前只保留 Port 接口和数据契约预留，不进入默认的 L1 上下文组装；只有后续真正启用 L5 能力时，才把它纳入 `L1 Context Builder` 的输入。

### 8.3 L2 工作记忆持久化策略

关键改进：L2 只承担“局部恢复”和“局部纠偏”职责，不把整份工作记忆都当作长期资产。每次迭代结束后增量 checkpoint 到 Redis staging；一旦形成可复用或需审计的产物（验收结果、证据卡、升级报告、关键纠偏结论），立即上提到 L3/L4，而不是继续只留在 L2。
...
1. L2 允许宕机丢失，设计目标是“减少重跑成本”，而不是“承担长期真理源”。
2. `draft_claims`、局部重写 query、临时评估分数等可只保留在 L2。
3. 原始检索片段若命中高敏 PII，可短暂保留在 L2 / staging 的受控引用中用于当前子任务内部处理，但不能直接晋升到 L3-Evidence。
4. 但一旦某轮结果被 `Verify` 验收、被 `StepGate` 消费，或会影响后续子任务/重规划，就必须以“已脱敏证据卡”的形式晋升到 L3-Evidence / L4 / MySQL 审计表。

### 8.4 L3 全局证据池共享机制

```
子任务 A 完成 → 先将已脱敏证据卡写入 Redis hot pool / staging
...
1. L3-Evidence 属于数据面，采用 Redis 先写热池 / staging、MySQL 异步归档的策略。
2. 只有通过 `PII 检测与脱敏` 的证据卡才能进入 L3-Evidence；`BLOCKED` 证据只保留受控引用，不参与跨子任务共享。
3. 在当前深搜场景下，允许极少量数据面 staging 丢失，并通过重检或补偿任务恢复；但这不适用于控制面状态。
...
| 控制面热缓存更新 | MySQL 提交后删除 Redis（cache-aside） | 热 key 可选延迟双删，优先保证正确性 |
...
6. 控制面写入成功后，作为 Redis 热缓存的唯一重建基准
...
1. 控制面字段先在 MySQL 事务内提交，SQLalchemy cache-after-commit 再删除对应 Redis 热缓存，遵循 cache-aside。
2. 不采用“先删缓存再写库”，避免并发读把旧库数据重新写回缓存。
3. 对极热 key（如 `task:{task_id}:state`）可选采用延迟双删：`写库 -> 删缓存 -> sleep(200~500ms) -> 再删一次`。
4. 当前领域接受极低概率的短暂脏缓存；若未来进入金融/库存级强一致场景，应升级为 binlog 驱动缓存失效或串行化更新，而不是单靠删缓存顺序。
...
5. **计划图边界**：如果 `planner` 内部使用 `networkx` 构建动态任务图，只允许把它当作短生命周期的“计划图编译器”；最终必须降维输出 JSON-safe `dag_spec` / `dag_fingerprint`，不能把 `DiGraph` 本体放进 LangGraph State。详见 [技术拆解 Section 2.7.1](./技术拆解.md#langgraph-dag)。
6. **等待点边界**：如果未来在 GlobalGraph 中引入 `interrupt()` / `Command(resume=...)` 作为等待语法糖，只允许在无副作用的 wait node 中使用；派发外部任务的节点（如 `executor`）不能直接 `interrupt()`，否则恢复时会重复派发。详见 [技术拆解 Section 2.8.3](./技术拆解.md#langgraph-dag)。
7. **执行模型边界**：当前主线不采用“单 worker + `Send()` 一次性跑完整个 Deep Search”；而是“编排 worker 分段执行 GlobalGraph + 执行 worker 独立跑 SubtaskGraph + `resume_orchestrator` 恢复推进”。详见 [技术拆解 Section 2.8.4](./技术拆解.md#langgraph-dag)。
...
6. **易错点提醒**：`networkx` 只负责计划图校验/分层/指纹，不负责运行时控制；`interrupt()` 只适合 Clarify 或纯等待节点，不适合带副作用的 dispatch 节点。更细节的坑点与示例见 [技术拆解 Section 2.7.1 和 2.8.3](./技术拆解.md#langgraph-dag)。
7. **模式取舍**：如果只是教学 demo，单 worker + `Send()` fan-out 可以成立；但当前生产主线明确选择“恢复式编排 + 独立子任务执行”的模式。详见 [技术拆解 Section 2.8.4](./技术拆解.md#langgraph-dag)。
...
2. 完成多路检索执行与证据汇聚
3. 完成 `Rerank → PII 检测与脱敏 → Compress` 证据后处理链路
4. 完成三维评估打分
5. 完成缺口诊断与纠偏重试
6. 完成输出三重校验
...
测试要求：子任务集成测试（Mock LLM + Mock 检索通道，验证完整闭环）；PII 脱敏测试（手机号、邮箱、员工编号等命中后分别进入 `MASKED`、`SUMMARY_ONLY`、`BLOCKED` 路径）；证据评估单测（三维打分计算、阈值判定、冲突检测）；最终汇总测试（冲突披露、降级输出、引用继承）；错误处理测试（LLM 超时、检索不可用时的降级行为）；数据面刷库测试（flush 成功、重复消费、补刷恢复）。
