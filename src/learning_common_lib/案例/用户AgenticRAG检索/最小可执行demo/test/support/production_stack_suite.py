...
try:
    ...
    demo_parent = Path(__file__).resolve().parents[2].parent
...
async def test_checkpoint_resume_recovery() -> dict:
    ...
    maintenance_service = build_maintenance_service(use_task_engine=True)
    graph_service = await build_global_graph_service_from_bundle(runtime, use_task_engine=True)
    request_id = build_request_id("sess_integration_checkpoint_resume", "请帮我整理差旅报销规则的变化")
    stale_at = utcnow() - timedelta(minutes=5)
    ...
    async with runtime.session_factory() as session:
        async with session.begin():
            await runtime.session_service.ensure_session(
                session,
                session_id="sess_integration_checkpoint_resume",
                tenant_id="demo-tenant",
                user_id="demo-user",
                initial_query="请帮我整理差旅报销规则的变化",
            )
            task = SearchTask(
                request_id=request_id,
                session_id="sess_integration_checkpoint_resume",
                ...
                kb_code="default",
                scope_json=None,
                original_query="请帮我整理差旅报销规则的变化",
                resolved_query="请帮我整理差旅报销规则的变化",
                task_profile_json={},
                status="PENDING",
                active_plan_version=0,
                budget_json={},
    ...
    async with runtime.session_factory() as session:
        task = await session.scalar(select(SearchTask).where(SearchTask.id == task_id))
        if task is None:
            raise AssertionError("Checkpoint resume test task missing before interrupt")
        initial_state = graph_service._build_initial_state(task, entry_action="intake")

    config = {"configurable": {"thread_id": f"deepsearch-task-{task_id}"}}
    interrupted = await graph_service.graph.ainvoke(
        initial_state,
        config=config,
        interrupt_after=["planner"],
    )
    if interrupted.get("next_action") != "clarify":
        raise AssertionError(f"Checkpoint resume test expected planner to stage clarify, got {interrupted}")

    checkpoint_state = await graph_service.graph.aget_state(config)
    if tuple(checkpoint_state.next) != ("clarify",):
        raise AssertionError(f"Checkpoint resume test expected pending clarify checkpoint, got {checkpoint_state}")

    async with runtime.session_factory() as session:
        async with session.begin():
            task = await session.scalar(select(SearchTask).where(SearchTask.id == task_id).with_for_update())
            if task is None:
                raise AssertionError("Checkpoint resume test task missing before recovery")
            task.original_query = "请帮我整理公司近 90 天差旅报销规则的变化"
            task.resolved_query = task.original_query
            task.updated_at = stale_at

    summary = await maintenance_service.recover_orchestration_gaps(stall_seconds=0)
    if summary["resumed"] < 1:
        raise AssertionError(f"Checkpoint resume test expected resumed >= 1, got {summary}")

    final_snapshot = None
    for _ in range(30):
        async with runtime.session_factory() as session:
            final_snapshot = await runtime.progress_service.build_snapshot(session, request_id)
        if final_snapshot.status == "WAITING_CLARIFICATION":
            break
        await asyncio.sleep(1)
    if final_snapshot is None or final_snapshot.status != "WAITING_CLARIFICATION":
        raise AssertionError(
            f"Checkpoint resume test expected WAITING_CLARIFICATION from checkpoint resume, got {getattr(final_snapshot, 'status', None)}"
        )
    clarification_request = final_snapshot.clarification_request
    clarification_reason_code = getattr(clarification_request, "reason_code", None)
    if clarification_reason_code != "missing_time_range":
        raise AssertionError(
            f"Checkpoint resume test expected missing_time_range clarify payload, got {clarification_request}"
        )
    return {
        "request_id": request_id,
        "summary": summary,
        "checkpoint_next": list(checkpoint_state.next),
        "final_status": final_snapshot.status,
...
async def run_offline_suite() -> dict[str, dict]:
    return {
        ...
        "fallback_late_result_guard": await test_fallback_ignores_late_result_and_staged_payload(),
        ...
        "reaped_run_payload_guard": await test_reaped_run_payload_is_rejected(),
        ...
        "finalize_degraded_guidance": await test_finalize_degraded_includes_guidance(),
...
async def main() -> None:
    ...
    try:
        ...
        summary.update(await run_offline_suite())
        summary["logs"] = {
