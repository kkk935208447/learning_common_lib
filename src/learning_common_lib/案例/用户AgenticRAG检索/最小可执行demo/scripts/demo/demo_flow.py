...
try:
    ...
    from ..setup.init_db import reset_tables
    from ..setup.seed_demo_kb import seed_demo_kb
except ImportError:
    import sys
    ...
    from 最小可执行demo.infrastructure.runtime_bundle import build_runtime_bundle
    from 最小可执行demo.scripts.setup.init_db import reset_tables
    from 最小可执行demo.scripts.setup.seed_demo_kb import seed_demo_kb
...
async def main() -> None:
    ...
    if snapshot.status == "WAITING_CLARIFICATION" and snapshot.clarification_request is not None:
        ...
        await resume_search_async(
            ...
            entry_action=(
                "planner"
                if snapshot.clarification_request.clarification_source == "PREPLAN"
                else "step_gate"
            ),
        )
        async with runtime.session_factory() as session:
