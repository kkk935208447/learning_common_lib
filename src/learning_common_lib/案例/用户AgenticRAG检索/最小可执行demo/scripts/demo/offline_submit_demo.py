...
try:
    ...
    from ...infrastructure.runtime_bundle import build_runtime_bundle, build_search_command_service
except ImportError:
    import sys
...
async def main() -> None:
    ...
    for idx in range(60):
        async with runtime.session_factory() as session:
            final_snapshot = await runtime.progress_service.build_snapshot(
                session,
                accepted.request_id,
            )
