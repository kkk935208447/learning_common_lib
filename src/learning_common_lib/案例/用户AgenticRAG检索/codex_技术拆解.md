# Agentic RAG 用户深度检索技术拆解（Codex 修订版）

本文件用于补充说明 [codex_架构设计规划](./codex_架构设计规划.md) 中最容易在实现阶段出现歧义的模块。规划文档负责给出整体决策和边界，这份技术拆解负责把关键模块展开到"可以直接开始写代码"的粒度。

## 1. 文档定位

建议把两份文档配合阅读：

1. [codex_架构设计规划](./codex_架构设计规划.md)：看整体原则、表设计、状态机、记忆系统、实施顺序。
2. 本文：看关键模块的落地规则、编排引擎选型、状态传递机制、并发适配和流程图。

本文不重复展开所有表字段和状态机定义，而是只解释那些"实现时最容易踩坑"的地方。

> 本文保留原始蓝图里多数细化流程、表格和示例，但统一修正为 MySQL-first、恢复编排驱动、租户隔离闭合的实现口径。
...
1. **GlobalGraph**（全局循环）：负责 Intake → Planner → Scheduler → StepGate → Replan/Output 的主流程。
2. **SubtaskGraph**（子任务闭环）：负责 Router → Cache → Rewrite → Retrieval → Eval → Verify/Retry 的局部循环。
3. **嵌套方式**：GlobalGraph 的 `executor` 节点负责登记 `execution_id` 并分发 Celery；Celery Worker 内部运行 `SubtaskGraph`，子图执行完毕后把结果写回 MySQL/Redis，再恢复 GlobalGraph。

```mermaid
flowchart TB
    subgraph GlobalGraph["GlobalGraph（全局循环）"]
        intake["intake<br/>任务画像"]
        planner["planner<br/>DAG 生成"]
        scheduler["scheduler<br/>就绪任务分发"]
        executor["executor<br/>登记 execution_id + 分发 Celery"]
    ...
    loop_guard -- "超限" --> fallback --> output
    step_gate -- "全部完成" --> output

    executor -.-> router
    router --> cache_probe --> rewrite --> retrieve --> post_process --> evaluate
    evaluate -- "充分" --> verify
    verify -- "通过" --> executor
    verify -- "不通过" --> retry --> router
    evaluate -- "不充分" --> retry
```

> **说明**：图中 `executor → step_gate` 表示子任务完成后恢复 `GlobalGraph` 再进入 `step_gate`，不表示在 LangGraph 节点内同步阻塞等待 Celery 返回。
...
class GlobalState(TypedDict, total=False):
    ...
    budget_consumed: dict                 # 已消耗量，原子递增
    policy: dict                          # threshold_profile / stop_rules
    global_iteration: int                 # 当前全局循环轮次
    max_global_iterations: int            # 全局循环上限，默认 3

    # ── DAG 规划 ──
    active_plan_version: int
    dag_json: dict                        # 当前活动 DAG 结构
    dag_fingerprint: str                  # DAG 指纹哈希
    historical_fingerprints: list[str]    # 历史版本指纹，用于循环检测
    replan_reason: str                    # 最近一次重规划原因

    # ── 子任务调度 ──
    subtask_results: dict[str, dict]      # subtask_code → 执行结果
    ready_queue: list[str]                # 当前就绪的子任务 code 列表
    completed_codes: list[str]            # 已完成的子任务 code 列表
    failed_codes: list[str]              # 已失败的子任务 code 列表

    # ── 证据池 ──
    evidence_pool: list[dict]             # 全局证据卡列表
    conflict_pairs: list[tuple[str, str]] # 冲突证据对
    ...
    # ── 流程控制 ──
    next_action: Literal["schedule", "replan", "output", "fallback"]
```

字段设计原则：

1. 控制平面字段（`budget`、`policy`）在 `intake` 节点初始化，后续节点只读不写。
2. `subtask_results` 是 Blackboard 的核心载体，子任务完成后由 `executor` 节点写入。
...
def loop_guard_router(state: GlobalState) -> str:
    """检查全局迭代次数是否超限"""
    if state["global_iteration"] >= state["max_global_iterations"]:
        return "fallback"
    # 检查 DAG 指纹是否重复（循环检测）
...
class SubtaskState(TypedDict, total=False):
    ...
    max_iterations: int                   # 局部循环上限，默认 3
    timeout_ms: int
    budget_slice: dict                    # 分配给本子任务的预算切片

    # ── 缓存 ──
    cache_hit: bool
    cached_evidence: list[dict]           # 缓存命中的证据
    cache_fresh: bool                     # 缓存是否仍在有效期

    # ── 改写与检索 ──
    rewritten_queries: dict[str, str]     # route_type → rewritten_query
    raw_results: list[dict]               # 多路检索原始结果
    fused_results: list[dict]             # 融合排序后结果

    # ── 证据评估 ──
    evidence_cards: list[dict]            # 压缩后的证据卡
    eval_scores: dict                     # coverage / conflict / confidence / total
    eval_verdict: Literal["SUFFICIENT", "INSUFFICIENT", "NEED_CLARIFICATION"]
    gap_type: str                         # coverage_gap / conflict_gap / confidence_gap
    gap_detail: str

    # ── 输出校验 ──
    draft_answer: str
    claims: list[dict]
    verify_result: dict                   # factual / sensitive / citation 三维结果
    verify_passed: bool

    # ── 流程控制 ──
    next_action: Literal["verify", "retry", "escalate", "clarify", "done"]
```

### 2.6 SubtaskGraph 条件边定义

SubtaskGraph 有三个关键条件边：
...
def verify_router(state: SubtaskState) -> str:
    ...
    if not result.get("factual_consistent"):
        return "retry_factual"      # 事实不一致 → 重新生成
    if result.get("has_sensitive"):
        return "retry_sensitive"    # 敏感信息 → 脱敏处理
    if not result.get("citation_aligned"):
        return "retry_citation"     # 引用不对齐 → 重新标注
    return "retry_factual"          # 兜底
```

条件边三：`local_guard` 迭代守卫
...
### 2.7 并行子任务执行：用 Send() 实现 DAG 并行分发

LangGraph 的 `Send()` 机制天然适合 DAG 并行分发场景。

核心思路：

1. `scheduler` 节点从 DAG 中找出所有入度为 0 的就绪子任务。
2. 对每个就绪子任务，用 `Send("executor", subtask_state)` 并行分发。
3. LangGraph 自动并行执行所有 `Send` 目标，结果汇聚后进入 `step_gate`。

```python
from langgraph.constants import Send

def scheduler_node(state: GlobalState) -> list[Send]:
    """找出就绪子任务，并行分发到 executor"""
    ready = compute_ready_tasks(state["dag_json"], state["completed_codes"])
    ready = ready[:MAX_CONCURRENT]  # 并行度限制，MAX_CONCURRENT 从 AppSettings 读取
    sends = []
    for subtask_code in ready:
        subtask_info = state["dag_json"]["tasks"][subtask_code]
        sends.append(Send("executor", {
            ...
            "route_hints": subtask_info.get("route", []),
            "iteration": 0,
            "max_iterations": 3,
            "timeout_ms": subtask_info.get("timeout_ms", 30000),
        }))
    return sends
```
...
1. `Send()` 的并行度受 LangGraph 运行时控制，不需要手动管理线程池。
2. 每个并行分支拿到的是独立的状态切片，不会互相干扰。
3. 所有分支完成后，结果自动汇聚回 GlobalState，由 `step_gate` 统一判定。

`execution_id` 不在 `scheduler` 节点生成，而是在 `executor` 领取 READY 子任务时生成并回写 MySQL；这样可以把 `READY → RUNNING` 的状态切换与幂等标识绑定在同一个原子操作里。
...
2. 进程崩溃后，用 `graph.invoke(None, config={"configurable": {"thread_id": thread_id}})` 从最近 checkpoint 恢复。
3. Redis Key 格式由 LangGraph 内部管理，无需手动维护。
4. MySQL 继续承担业务真理源与恢复判定，Checkpoint 只负责 LangGraph 运行时状态，不替代业务持久化。

恢复流程：

1. API 收到重试请求或 Worker 重启后，用原始 `thread_id` 调用 `graph.invoke`。
...
2. `SubtaskGraph`：`tenant:{tenant_id}:task:{task_id}:plan:{plan_version}:subtask:{subtask_code}`

### 2.9 LangGraph 与 Celery 的分工边界

两者的定位完全不同，不要混淆：

1. **LangGraph 管编排逻辑**：节点定义、条件路由、状态传递、并行分发、checkpoint。运行在 API 进程或轻量 Worker 中，本身不做重型计算。
2. **Celery 管重型异步任务**：LLM API 调用、向量检索、ES 检索、证据压缩等 IO/CPU 密集操作。运行在独立 Worker 进程中，按队列隔离。

交互方式：

1. `executor` 节点为 READY 子任务生成 `execution_id`，条件更新 MySQL 状态为 `RUNNING`，再 dispatch Celery task。
2. Celery Worker 内部运行 `SubtaskGraph`，完成后把结果写回 MySQL/Redis，并返回标准结果包。
...
```python
@celery_app.task(bind=True, queue="llm_jobs",
                 soft_time_limit=30, time_limit=35)
def llm_call_task(self, prompt, model, **kwargs):
    try:
        return call_llm(prompt, model, **kwargs)
    except SoftTimeLimitExceeded:
        logger.warning("LLM call soft timeout, returning partial result")
        return {"error": "llm_timeout", "partial": True}
```
...
def resume_orchestrator(result: SubtaskResultEnvelope):
    ...
    if not persist_result_if_current(result):
    ...
    try:
        ...
        global_graph.invoke(None, config={"configurable": {"thread_id": thread_id}})
    except Exception:
...
> **注意**：Checkpointer 写入失败意味着崩溃恢复能力暂时丧失。应配合告警（参见 `codex_架构设计规划.md` Section 12.3），连续写入失败 > 3 次时触发告警。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `status` | string | PENDING/PLANNING/EXECUTING/COMPLETED/FAILED/DEGRADED |
...
3. 子任务完成后设置 2h TTL，避免 Redis 内存无限增长。
...
1. 子任务的 `post_process` 节点完成证据压缩后，将证据卡写入 Redis 和 MySQL。
2. Redis 写入用于跨子任务实时共享（后续子任务可以读取前序子任务的证据）。
3. MySQL 写入用于持久化和审计。
...
首版确认方案：**Celery 结果回写 + 恢复编排**，不引入 Redis Stream，也不使用 Celery `on_success` 直接链式调度下游。

原因：

1. Redis Stream 需要额外的消费者组管理、ACK 机制、死信处理，首版复杂度过高。
2. 让 Celery `on_success` 直接推进下游，会和 LangGraph 编排形成双控制面，状态恢复会变复杂。
3. 采用“Worker 回写结果 → `resume_orchestrator` 恢复 GlobalGraph”后，编排权只保留在 LangGraph/MySQL 这一侧，恢复路径更清晰。

具体实现：

```python
@celery_app.task(bind=True, queue="execute_jobs")
def execute_subtask(
    self,
    tenant_id: str,
    task_id: int,
    ...
    """Celery Worker 执行子任务"""
    # 1. 调用 SubtaskGraph
    result = subtask_graph.invoke(build_subtask_state(...))
    # 2. 写回结果到 Redis + MySQL
    envelope = save_subtask_result(
        tenant_id=tenant_id,
        task_id=task_id,
...
`resume_orchestrator` 中必须包含 stale result fencing、READY 判定和死锁检测逻辑：
...
def resume_orchestrator(result: SubtaskResultEnvelope):
    ...
    if not persist_result_if_current(result):
        # 已发生 replan 或 execution_id 不匹配，视为 stale result
        logger.info("stale result ignored", extra=result)
        return

    active_plan_version = get_active_plan_version(tenant_id, task_id)
    if active_plan_version != plan_version:
        logger.info("result belongs to old plan version", extra=result)
        return

    ready_codes = decrement_indegree_or_rebuild(
        tenant_id=tenant_id,
        task_id=task_id,
        plan_version=plan_version,
        completed_code=subtask_code,
    )
    ...
    subtasks = get_all_subtasks(tenant_id, task_id, plan_version)
    completed = [s for s in subtasks if s.status in ("COMPLETED", "SKIPPED")]
    failed = [s for s in subtasks if s.status == "FAILED"]
    running = [s for s in subtasks if s.status == "RUNNING"]
    pending = [s for s in subtasks if s.status in ("PENDING", "READY")]

    if not pending and not running:
        # 全部终结，恢复 GlobalGraph 并进入 step_gate
        thread_id = f"tenant:{tenant_id}:task:{task_id}"
        global_graph.invoke(None, config={"configurable": {"thread_id": thread_id}})
        return

    if not running and pending:
        logger.error(f"Deadlock detected: task={task_id}, "
                     f"pending={[s.subtask_code for s in pending]}, "
                     f"failed={[s.subtask_code for s in failed]}")

        if failed:
            trigger_replan.apply_async(args=[tenant_id, task_id, "deadlock_failed_dependency"])
        else:
            trigger_fallback.apply_async(args=[tenant_id, task_id, "deadlock_circular_dependency"])
```

### 3.8 Scheduler 就绪任务发现机制

完整流程（含 stale result fencing 和死锁检测分支）：

```mermaid
flowchart TD
    A["子任务完成"] --> B["写回 MySQL + Redis"]
    B --> B2["投递 resume_orchestrator"]
    B2 --> C{"plan_version / execution_id 仍有效？"}
    C -- "否" --> C2["标记 stale result<br/>仅落审计"]
    C -- "是" --> D["Lua 递减入度<br/>缓存 miss 则回退 MySQL"]
    D --> E{"有新就绪任务？"}
    E -- "有" --> F["claim READY 并 dispatch 到 Celery"]
    E -- "没有" --> G{"所有子任务都终结？"}
    G -- "是" --> H["恢复 GlobalGraph<br/>进入 step_gate"]
    G -- "否" --> G2{"有 RUNNING 任务？"}
    G2 -- "有" --> I["等待其他子任务完成"]
    G2 -- "没有" --> DL["⚠️ 死锁检测"]
    DL --> DL2{"有 FAILED 前置？"}
    DL2 -- "是" --> K["触发 Replan"]
    DL2 -- "否" --> M["触发 Fallback<br/>（DAG 环路）"]
    H --> J{"step_gate 结果"}
    J -- "output" --> N["生成最终回答"]
    J -- "replan" --> K
    J -- "schedule" --> L["继续 scheduler"]
```
...
1. Checkpointer 使用独立的 Redis DB（例如 `db=1`），业务状态使用 `db=0`。
2. `GlobalGraph` 的 `thread_id` 使用 `tenant:{tenant_id}:task:{task_id}`，保证跨租户恢复时不会串数据。
3. `SubtaskGraph` 的 `thread_id` 使用 `tenant:{tenant_id}:task:{task_id}:plan:{plan_version}:subtask:{subtask_code}`，避免不同计划版本或不同子任务的 checkpoint 互相覆盖。
...
# Checkpointer Redis（db=1）—— 由 LangGraph RedisSaver 管理
# 连接参数在 RedisSaver 初始化时传入，与业务池隔离
```
...
async def cleanup_task_redis_keys(tenant_id: str, task_id: int, subtask_codes: list[str]):
    ...
    # 或在 db=1 上用 SCAN + EXPIRE 批量设置 TTL
...
2. 但 Redis 中的热缓存可能不完整（TTL 过期或 AOF 截断），需要从 MySQL 重建：
   - 启动时扫描 `search_tasks` 表中 `status IN ('EXECUTING', 'PLANNING')` 的任务。
...
4. 任务粒度是"一次 LLM 调用"或"一次检索执行"，单任务耗时 1-30 秒，Celery 的 prefork 模型完全适配。
...
| 任务类型 | 队列 | 推荐 Pool | 原因 |
|----------|------|-----------|------|
| LLM API 调用 | `llm_jobs` | celery-aio-pool | IO 等待为主，原生 asyncio 支持，与 LangGraph async node 兼容 |
| 向量检索 / ES 检索 | `retrieval_jobs` | celery-aio-pool | 同上，网络 IO 为主 |
| 证据压缩 / NLI 推理 | `compute_jobs` | prefork | CPU 密集，需要独立进程 |
| 子任务编排 | `orchestrate_jobs` | solo 或 prefork(1) | 轻量，只做 LangGraph 调度 |

Worker Lane 部署建议：

```bash
# Lane 1: IO 密集任务（celery-aio-pool，需先 pip install celery-aio-pool）
export CELERY_CUSTOM_WORKER_POOL='celery_aio_pool.pool:AsyncIOPool'
celery -A app worker -Q llm_jobs,retrieval_jobs -P custom -c 50

# Lane 2: CPU 密集任务
celery -A app worker -Q compute_jobs -P prefork -c 4
...
2. LangGraph 自身支持 async node，后续可以让部分轻量 IO（如 Redis 读写）直接在 LangGraph 节点内 async 执行，减少 Celery 调度开销。
3. 如果 Redis Broker 成为瓶颈，可以切换到 RabbitMQ 或 Kafka，Celery 的 Broker 是可插拔的。
4. K8s 扩展时，按 Lane 独立部署 Deployment，通过 HPA 按队列积压量自动扩缩。
...
celery_app.conf.update(
    ...
    # --- 队列路由 ---
    task_routes={
        "workers.llm_*": {"queue": "llm_jobs"},
        "workers.retrieval_*": {"queue": "retrieval_jobs"},
        "workers.compute_*": {"queue": "compute_jobs"},
        "workers.orchestrate_*": {"queue": "orchestrate_jobs"},
    },
...
> **celery-aio-pool 说明**：IO 密集型队列（`llm_jobs`、`retrieval_jobs`）推荐使用第三方库 `celery-aio-pool`（`pip install celery-aio-pool`），它为 Celery Worker 提供原生 asyncio 事件循环池，任务函数可以直接定义为 `async def`。相比 gevent 的 monkey-patch 方案更干净，与 LangGraph 的 async node 和 `asyncio` 生态（如 `httpx`、`aiohttp`）天然兼容。启动方式：先设置环境变量 `export CELERY_CUSTOM_WORKER_POOL='celery_aio_pool.pool:AsyncIOPool'`，再以 `-P custom` 启动 Worker。
...
核心原则：子任务循环在自己的预算切片内自治，只有两种情况才向全局循环升级。
...
escalate_report = {
    ...
    "gap_detail": "缺少 2024Q4 华东区新签客户明细",
    "evidence_collected": ["card_001", "card_003"],
    "suggestion": "建议拆分为两个更细粒度的子任务，或补充结构化数据源"
}
```
...
1. 子任务完成 `post_process` 后，将证据卡写入全局证据池（Redis Hash + MySQL 双写）。
2. 后续子任务在 `router` 节点启动时，先读取全局证据池，检查是否有可复用的证据。
3. 如果前序子任务的证据已经覆盖了当前子任务的部分需求，可以跳过对应的检索路由。
4. `step_gate` 节点读取全局证据池做汇总评估，决定是否需要重规划。
...
3. 首版的全局循环上限只有 3 次，等待当前批次完成的额外延迟可以接受。

具体流程：

1. `step_gate` 判定需要 Replan 后，将 `next_action` 设为 `replan`。
2. 等待当前所有正在执行的子任务完成（通过 Celery result backend 轮询或回调）。
...
3. 冲突对写入 `evidence_cards.conflicts_with_json` 和 GlobalState 的 `conflict_pairs`。
...
4. 如果不可调和冲突涉及核心结论，触发 `NEED_CLARIFICATION`，请求用户指定以哪个数据源为准。
...
1. `max_global_iterations` 默认为 3，超过后强制进入 Fallback。
