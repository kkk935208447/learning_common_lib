...
flowchart TB
    ...
    planner -- "前置信息不足" --> clarify --> wait_clarify
    wait_clarify -- "source=PREPLAN" --> planner
    wait_clarify -- "source=STEP_GATE" --> step_gate
    executor --> wait_subtasks
    step_gate -- "下一步" --> scheduler
    step_gate -- "需要用户补充" --> clarify
...
def execute_subtask(
    ...
    append_task_event(task_id, event_type, envelope)
    publish_sse_event(task_id, sse_type, envelope)
