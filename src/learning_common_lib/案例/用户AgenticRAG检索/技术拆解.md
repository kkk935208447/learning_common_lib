...
1. **GlobalGraph**（全局循环）：负责 Intake → Planner → Scheduler → Executor → StepGate → Replan / Finalize 的主流程。
2. **SubtaskGraph**（子任务闭环）：负责 Router → Cache → Rewrite → Retrieval → Eval → Verify / Escalate 的局部循环。
...
4. **实现约束**：首版不依赖 `Send()` 的“自动汇聚到下一个节点”语义来承载异步 Celery 结果；真正的恢复入口只有 `resume_orchestrator`。
...
> **关键边界**：
>
> 1. `GlobalGraph` 在分发一批子任务后进入 `WAITING_SUBTASKS`，不在图内阻塞等待 Celery `.get()`。
...
sequenceDiagram
    ...
    CW->>DB: subtask_runs = COMPLETED or FAILED
    CW->>RO: 发送 SubtaskResultEnvelope
...
class GlobalState(TypedDict, total=False):
    ...
    max_clarification_count: int          # Clarify 上限，默认 1
...
class SubtaskState(TypedDict, total=False):
    ...
    # ── 证据评估 ──
    evidence_ref: dict | None             # 压缩后的证据引用
...
> **状态约束**：`SubtaskState` 只保存子图控制信息和数据库引用，证据卡全文、原始检索结果、完整 claims 列表全部放在 MySQL/Redis，不直接塞进 Checkpointer。
...
1. **LangGraph 管控制流**：节点定义、条件路由、状态恢复、重规划、最终汇总。运行在 API 进程或轻量 Worker 中，本身不持有大对象执行态。
2. **Celery 管执行闭环**：单个子任务内部的改写、检索、证据压缩、评估、校验在同一 Worker 内完成，避免 Celery 套 Celery。
...
def execute_subtask(self, execution_id, **kwargs):
    try:
        return run_subtask_graph(execution_id=execution_id, **kwargs)
    except SoftTimeLimitExceeded:
        logger.warning("Subtask soft timeout, returning degraded envelope")
        return {"error": "subtask_timeout", "partial": True}
```
...
1. 子任务的 `post_process` 节点完成证据压缩后，先将证据卡摘要写入 Redis 热池，并把完整 payload 写入 Redis staging。
2. Redis 热池用于跨子任务实时共享（后续子任务可以读取前序子任务的证据）。
3. MySQL 持久化改由 `persist_jobs` 异步批量刷入，用于审计、最终查询和补偿恢复。
...
def execute_subtask(
    ...
    """Celery Worker 执行整个子任务闭环"""
    result = subtask_graph.invoke(build_subtask_state(...))
    payload_ref = stage_data_plane_payload(
        tenant_id=tenant_id,
        execution_id=execution_id,
        result=result,
    )
    envelope = save_control_plane_result(
        tenant_id=tenant_id,
        task_id=task_id,
    ...
    append_task_event(task_id, "subtask_completed", envelope)
    publish_sse_event(task_id, "subtask.completed", envelope)
...
def resume_orchestrator(result: SubtaskResultEnvelope):
    ...
    if not is_current_execution(result):
        append_task_event(result["task_id"], "subtask_stale_ignored", result)
        return

    append_task_event(result["task_id"], "subtask_result_accepted", result)
...
>
> **Clarify 约束**：当 `step_gate` 选择 `clarify` 时，只允许生成一次结构化单选题；`clarification_count >= max_clarification_count` 时必须直接走 `fallback` 或显式默认项恢复，不允许再追问。
...
5. `WAITING_CLARIFICATION` 时必须发送 `task.waiting_clarification` 事件，payload 内包含 `question`、`options`、`default_option_id` 和 `expires_at`。
...
flowchart TD
    ...
    C -- "是" --> D["恢复 GlobalGraph checkpoint"]
...
| Retrieval Agent | `PlanNodeSpec`、L2/L3 证据引用 | `evidence_cards`、L2 工作记忆 | `active_plan_version` |
...
>
> **Clarify 工程约束**：`Clarification Agent` 只生成结构化单选题，请求体必须包含 `question`、`options[]`、`default_option_id` 和 `expires_at`；不得返回自由文本追问。
...
1. 子任务完成 `post_process` 后，先把证据卡摘要写入全局证据池（Redis Hash），完整 payload 写入 Redis staging，并由 `persist_jobs` 异步刷入 MySQL。
