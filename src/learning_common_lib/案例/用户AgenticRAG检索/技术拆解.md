...
| 任务类型 | 队列 | 推荐 Pool | 原因 |
|----------|------|-----------|------|
| LLM API 调用 | `llm_jobs` | gevent | IO 等待为主，gevent 协程可以用少量线程支撑高并发 |
| 向量检索 / ES 检索 | `retrieval_jobs` | gevent | 同上，网络 IO 为主 |
...
```bash
# Lane 1: IO 密集任务
celery -A app worker -Q llm_jobs,retrieval_jobs -P gevent -c 50
