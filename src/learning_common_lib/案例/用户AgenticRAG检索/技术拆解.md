...
这里的“挂起”是业务状态机语义，不是保留一个 Python 线程或协程在图里等待子任务结果。

执行路径必须理解为“退出当前图调用，再由外部事件重新进入”：

1. `scheduler` 计算 READY 批次，`executor` claim 子任务并 dispatch Celery。
2. `executor` 在 MySQL 中写入 `WAITING_SUBTASKS`，同时把最小控制状态交给 Checkpointer。
3. 当前这次 `await global_graph.ainvoke(...)` 调用到此结束；事件循环释放这个协程，不再持有等待中的 Future。
4. 子任务在 Celery Worker 内独立执行，Graph 不轮询、不 `.get()`、不保留等待线程。
5. Worker 完成后写回控制面结果，并投递轻量 `resume_orchestrator(SubtaskResultEnvelope)`。
6. `resume_orchestrator` 触发一次新的 `await global_graph.ainvoke(None, config={"configurable": {"thread_id": thread_id}})`，LangGraph 从最近 checkpoint 继续执行 `step_gate` / `scheduler`。

因此，`WAITING_SUBTASKS` 的本质是：

1. 任务外部可见状态处于等待中。
2. 图内当前调用已经结束，不再占住等待线程。
