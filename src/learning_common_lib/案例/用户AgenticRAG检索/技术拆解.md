...
4. `Checkpointer`：内置 Redis/SQLite/Postgres 持久化，崩溃后从最近 checkpoint 恢复。
...
3. **嵌套方式**：GlobalGraph 的 `executor` 节点内部调用 `SubtaskGraph.invoke(subtask_state)`，子图执行完毕后将结果写回全局状态。
...
flowchart TB
    subgraph GlobalGraph["GlobalGraph（全局循环）"]
        ...
        executor["executor<br/>调用 SubtaskGraph"]
...
2. 进程崩溃后，用 `graph.invoke(None, config={"configurable": {"thread_id": task_id}})` 从最近 checkpoint 恢复。
3. Redis Key 格式由 LangGraph 内部管理，无需手动维护。
4. 如果后续需要更强的持久化保证，可以切换到 `PostgresSaver`。
...
1. LangGraph 节点内部 dispatch Celery task：`result = celery_task.apply_async(...).get(timeout=30)`。
2. Celery task 执行完毕后返回结果，LangGraph 节点将结果写入 State。
3. LangGraph 继续根据 State 决定下一个节点。
...
sequenceDiagram
    ...
    LG->>CB: apply_async(llm_call_task, args)
    CB->>CW: 分发到 llm_jobs 队列
    CW->>LLM: 调用 LLM API
    LLM-->>CW: 返回结果
    CW-->>CB: 标记完成
    CB-->>LG: .get() 获取结果
    LG->>LG: 更新 State，进入下一节点
```
...
#### 2.10.3 SubtaskGraph 失败时 executor 节点的捕获

`executor` 节点调用 SubtaskGraph 时，必须捕获子图异常并写入 `GlobalState.failed_codes`，而不是让异常冒泡导致整个 GlobalGraph 崩溃：

```python
async def executor_node(state: GlobalState, subtask_input: dict) -> GlobalState:
    subtask_code = subtask_input["subtask_code"]
    try:
        result = await asyncio.wait_for(
            subtask_graph.ainvoke(subtask_input),
            timeout=subtask_input.get("timeout_ms", 30000) / 1000
        )
        state["subtask_results"][subtask_code] = result
        state["completed_codes"].append(subtask_code)
    except (asyncio.TimeoutError, Exception) as e:
        logger.error(f"SubtaskGraph failed: {subtask_code}", exc_info=True)
        state["failed_codes"].append(subtask_code)
        state["subtask_results"][subtask_code] = {
            "status": "FAILED",
            "error": str(e),
            "evidence_cards": [],  # 保留已收集的部分证据
        }
    return state
```
...
> **注意**：Checkpointer 写入失败意味着崩溃恢复能力暂时丧失。应配合告警（参见架构设计规划 Section 12.3），连续写入失败 > 3 次时触发告警。
...
所有 Key 统一前缀 `rag:search:`，按功能域分层：

| 功能域 | Key 模式 | 数据类型 | TTL |
|--------|----------|----------|-----|
| 全局任务状态 | `rag:search:task:{task_id}:state` | Hash | 任务完成后 24h |
| DAG 入度表 | `rag:search:task:{task_id}:plan:{ver}:indegree` | Hash | 计划切换后 1h |
| 子任务工作记忆 | `rag:search:subtask:{task_id}:{code}:memory` | Hash | 子任务完成后 2h |
| 全局证据池 | `rag:search:task:{task_id}:evidence` | Hash | 任务完成后 24h |
| 预算消耗 | `rag:search:task:{task_id}:budget` | Hash | 任务完成后 24h |
...
Key：`rag:search:task:{task_id}:state`
...
4. 任务进入终态后，同步写入 MySQL，Redis 仅保留热缓存。

### 3.3 DAG 入度表（Redis Hash + Lua 脚本）

Key：`rag:search:task:{task_id}:plan:{ver}:indegree`
...
Key：`rag:search:subtask:{task_id}:{code}:memory`
...
Key：`rag:search:task:{task_id}:evidence`
...
Key：`rag:search:task:{task_id}:budget`
...
首版确认方案：**Celery 回调链**，不引入 Redis Stream。

原因：

1. Redis Stream 需要额外的消费者组管理、ACK 机制、死信处理，首版复杂度过高。
2. Celery 的 `on_success` 回调已经能满足"子任务完成 → 触发 Scheduler"的需求。
3. 中小企业并发度下，Celery 回调的延迟（毫秒级）完全可接受。

具体实现：

```python
@celery_app.task(bind=True, queue="execute_jobs")
def execute_subtask(self, task_id: int, subtask_code: str, plan_version: int):
    """Celery Worker 执行子任务"""
    # 1. 调用 SubtaskGraph
    result = subtask_graph.invoke(build_subtask_state(...))
    # 2. 写回结果到 Redis + MySQL
    save_subtask_result(task_id, subtask_code, result)
    return {"task_id": task_id, "subtask_code": subtask_code, "status": "done"}

@execute_subtask.on_success.connect
def on_subtask_done(sender, result, **kwargs):
    """子任务完成后触发 Scheduler"""
    task_id = result["task_id"]
    subtask_code = result["subtask_code"]
    # 原子递减入度，获取新就绪的子任务
    ready_codes = decrement_indegree_lua(task_id, subtask_code)
    if ready_codes:
        for code in ready_codes:
            execute_subtask.apply_async(args=[task_id, code, ...])
    else:
        # 没有新就绪任务，检查是否全部完成（含死锁检测）
        check_plan_completion.apply_async(args=[task_id])
```

`check_plan_completion` 中必须包含死锁检测逻辑（需求文档明确要求）：

```python
@celery_app.task(queue="orchestrate_jobs")
def check_plan_completion(task_id: int):
    """检查计划是否完成，包含死锁检测"""
    plan_version = get_active_plan_version(task_id)
    subtasks = get_all_subtasks(task_id, plan_version)
    ...
    if not pending and not running:
        # 全部终结，触发 step_gate
        trigger_step_gate.apply_async(args=[task_id])
        return

    if not running and pending:
        # ⚠️ 死锁：没有任务在跑，也没有新就绪任务，但仍有 PENDING
        # 原因：DAG 依赖关系形成环，或某个前置任务 FAILED 但未正确传播
        ...
        if failed:
            # 有失败的前置任务导致下游永远无法就绪 → 触发 Replan
            trigger_replan.apply_async(args=[task_id, "deadlock_failed_dependency"])
        else:
            # DAG 本身有环（不应发生，但防御性处理）→ 直接 Fallback
            trigger_fallback.apply_async(args=[task_id, "deadlock_circular_dependency"])
```
...
```mermaid
flowchart TD
    A["子任务完成"] --> B["on_success 回调触发"]
    B --> C["Lua 脚本原子递减入度"]
    C --> D{"有新就绪任务？"}
    D -- "有" --> E["dispatch 就绪子任务到 Celery"]
    D -- "没有" --> F{"所有子任务都终结？"}
    F -- "是" --> G["触发 step_gate 判定"]
    F -- "否" --> F2{"有 RUNNING 任务？"}
    F2 -- "有" --> H["等待其他子任务完成"]
    F2 -- "没有" --> DL["⚠️ 死锁检测"]
    DL --> DL2{"有 FAILED 前置？"}
...
2. `thread_id` 统一使用 `task_id` 的字符串形式，保证崩溃恢复时能找到正确的 checkpoint。
3. SubtaskGraph 的 `thread_id` 使用 `{task_id}:{subtask_code}`，避免不同子任务的 checkpoint 互相覆盖。
...
def get_indegree_from_redis_or_mysql(task_id: int, plan_version: int) -> dict:
    """读取 DAG 入度表，Redis 不可用时回退 MySQL"""
    try:
        key = f"rag:search:task:{task_id}:plan:{plan_version}:indegree"
    ...
    return compute_indegree_from_mysql(task_id, plan_version)
...
```python
async def cleanup_task_redis_keys(task_id: int, subtask_codes: list[str]):
    """任务完成后为所有相关 Redis Key 设置 TTL"""
    ttl_24h = 86400  # 24 小时

    pipe = business_redis.pipeline()
    # 业务 Key
    pipe.expire(f"rag:search:task:{task_id}:state", ttl_24h)
    pipe.expire(f"rag:search:task:{task_id}:evidence", ttl_24h)
    pipe.expire(f"rag:search:task:{task_id}:budget", ttl_24h)
    # 子任务工作记忆（较短 TTL）
    for code in subtask_codes:
        pipe.expire(f"rag:search:subtask:{task_id}:{code}:memory", 7200)  # 2h
    pipe.execute()
...
3. 收集所有已完成子任务的结果，写入全局证据池。
4. `replan` 节点基于已有证据和失败信息，生成新版本 DAG。
