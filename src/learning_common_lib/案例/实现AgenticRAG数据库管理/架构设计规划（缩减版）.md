...
- `storage_status` enum(`READY`, `DELETE_PENDING`, `DELETED`, `FAILED`)
...
6. `storage_key` 首版直接按 `document_id/version_no` 维度命名，不做按 `file_hash` 共享对象。
...
1. 文件必须先成功写入正式 `storage_key` 并通过最小可读性校验，再进入事务写 `document_versions` 与 `PARSE_REQUESTED`。
...
sequenceDiagram
    ...
    A->>O: 最小可读性校验
    A->>M: 事务内写 documents/document_versions(storage_status=READY)
    A->>X: 事务内写 PARSE_REQUESTED
...
2. 如果 MySQL 事务失败，`DocumentCommandService` 要 best-effort 删除刚写入的对象，避免无主文件。
3. 在线查询接口读取内容时，必须以 `documents.active_version_id` 为准；管理/调试接口可以按 `version_id` 直读单版本状态。
...
3. 只有对象写入成功并通过最小可读性校验后，版本才能以 `storage_status=READY` 进入事务，并写入 `PARSE_REQUESTED`。
4. 如果上传成功但事务失败，必须对对象做 best-effort 回收，避免产生“OSS 里有文件、MySQL 里无版本”的孤儿对象。
5. Cleaner 删除对象时只能按版本归属处理，不能按 `file_hash` 猜测复用关系。
