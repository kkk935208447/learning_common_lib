...
10. [技术拆解：文件指纹与对象存储生命周期](./技术拆解.md#file-hash-storage)
...
6. **Redis 锁只负责跨 Pod 协调**，例如防止同一版本被多个 Worker 并发处理，或保障 Janitor(清洁器) 单实例执行。
7. **Parser / Index / Cleaner(清除器) / Janitor(清洁器) 都要幂等**，任何阶段都要允许重复消费和重试。
...
11. **Outbox 仍然保留**。即使改用 Celery，也不能在数据库事务里直接发任务，仍需先落 `outbox_events` 再由 `Dispatcher`(调度器) 调用 Celery 发任务。
12. **Chunk 主键必须稳定可重建**。跨 MySQL、Milvus、ES 一律使用稳定 `chunk_uid`，禁止依赖数据库自增主键做投影主键。
13. **历史版本默认仅在 MySQL 中保留审计记录**。旧版本在激活切换后`异步清理(即 Cleaner)`投影层，避免在线检索混入旧内容。
14. **Janitor 不只做对账**。还要负责索引维护任务的调度入口，例如低峰期触发 Compact(压缩) / Rebuild(重建)。
...
- `staging_storage_key` varchar(512) null
- `storage_status` enum(`PENDING_PROMOTE`, `READY`, `DELETE_PENDING`, `DELETED`, `FAILED`) not null
...
6. `storage_status` 用于明确对象是否已经从临时落点归档到版本正式对象，避免 Parser 在源文件尚未就绪时抢跑。
7. `staging_storage_key` 只在上传完成但对象还未 promote/校验完成时短暂存在，正常情况下进入 `READY` 后应被清空。
...
3. Dispatcher 只等待“Broker 已接受发布请求”这一层确认；不等待 Worker 实际消费，更不等待任务执行完成。
4. 这样可以避免“数据库更新成功，但任务没有入队”的一致性问题。
5. `publish_status=SENT` 仅表示发布成功，不表示业务处理成功；最终仍以版本状态机和任务执行结果为准。
6. `publish_status=SENT` 的历史记录应由低频 housekeeping 任务定期清理或归档，避免 `outbox_events` 无限增长。

这部分细化规则见 [技术拆解：Outbox 发布确认与 ack 边界](./技术拆解.md#outbox-publish-ack)。
...
### 5.8 `file_hash` 与 OSS对象存储策略

这一块必须在规划阶段直接定死，否则实现时会出现“秒传”“去重”“清理归属”“解析抢跑”互相打架的问题。

首版建议规则：

1. `file_hash` 使用文件原始字节流的 `sha256`，在 API 接收流时边上传边计算；同时持久化 `file_size`，`mime_type` 以服务端嗅探结果为准，不信任客户端声明。
2. `file_hash` 的首要用途是判断**同一逻辑文档**是否发生内容变化，它不是 `documents` 的业务主键，也不是对象存储的共享主键，更不是“语义内容摘要”。
3. 同一个 `document_id` 上传与当前活动版本相同 `file_hash` 时，可以判定为幂等更新，直接返回当前活动版本，不再重复解析和索引。
4. 如果当前活动版本之外的历史版本曾经出现过相同 `file_hash`，也不要直接复用旧 `version_id`；是否创建新版本仍以“当前活动版本是否变化”和当前解析配置、审计需求为准。
5. 不同 `document_id` 即使 `file_hash` 相同，首版也**允许共存**，但不做跨文档版本复用、对象复用或引用计数共享，避免对象归属与删除条件复杂化。
6. 上传阶段一律先写 `staging_storage_key`，命名建议为 `staging/{yyyyMMdd}/{request_id}`；事务内创建版本时预留正式 `storage_key`，推荐按 `kb/{kb_id}/doc/{document_id}/ver/{version_no}/source` 或等价的版本归属路径命名。
7. `storage_key` 表达的是“该版本拥有的正式对象地址”，`staging_storage_key` 表达的是“临时落点”；不要依赖对象存储 rename 的原子语义，真实实现应采用 `copy -> verify -> delete temp` 或“预留正式 key 后直接写入”的方式落地。
8. 只有当对象校验通过且 `storage_status=READY` 后，系统才能写入 `PARSE_REQUESTED`；Parser 不读取 `PENDING_PROMOTE`、`FAILED`、`DELETED` 状态的对象。
9. Cleaner 只按版本归属删除 `storage_key`，不能按 `file_hash` 推断“相同内容即可共删”，也不能靠 hash 前缀扫描对象。
10. 历史版本源文件要单独定义保留策略：建议默认永久保留当前活动版本，历史版本按“保留最近 N 版”或“保留 N 天”二选一，不要一边强调审计/重建，一边在切换成功后立即硬删全部旧源文件。
11. 如果未来要支持真正的跨文档秒传，再新增 `stored_objects` / `blob_objects` 表维护对象所有权、租户隔离、引用计数和删除条件，不要在首版隐式复用对象。
12. 对象存储首版也应满足最小安全基线：私有 bucket、服务端加密、扩展名与 MIME 白名单、单文件大小上限、只通过后端签发临时访问凭证。
...
`document_versions` 建议拆成四组状态，而不是塞进一个字段：
...
1. 创建 `documents`
2. 创建 `document_versions(parse=PENDING, index=PENDING, visibility=STAGED, storage_status=PENDING_PROMOTE)`
3. 对象完成 promote/校验后更新 `storage_status=READY`
4. Outbox 写入 `PARSE_REQUESTED`
5. Parser 成功后 `parse=SUCCESS`
6. Outbox 写入 `INDEX_REQUESTED`
7. Indexer 双写 Milvus / ES 成功后
8. 将该版本切换为 `ACTIVE`
9. 更新 `documents.active_version_id`
...
2. 当前 `active_version_id` 或所有未清理版本打上 `visibility_status=DELETE_PENDING`；需要回收源文件时同步进入 `storage_status=DELETE_PENDING`
...
下图只展示成功主路径；失败补偿、回滚与 Janitor 修复见后续章节。
...
sequenceDiagram
    ...
    participant X as Outbox Table(MySQL)
    ...
    Note over M,X: outbox_events 与业务数据同库同事务提交
    U->>A: 上传文件
    A->>O: 写入 staging object + 计算 sha256
    A->>M: 事务 1 写 documents/document_versions(storage_status=PENDING_PROMOTE)
    A->>O: copy/verify 到正式 storage_key
    A->>M: 事务 2 更新 storage_status=READY
    A->>X: 同事务写 PARSE_REQUESTED
    D->>X: 拉取 PARSE_REQUESTED
    D->>C: apply_async(parse task)
    C->>P: 分发 parse 任务
    P->>O: 读取源文件
    P->>M: 事务内 upsert document_chunks + parse_status=SUCCESS
    P->>X: 同事务写 INDEX_REQUESTED
    D->>X: 拉取 INDEX_REQUESTED
    ...
    I->>M: 事务内更新 projection_sync_status/index_status=SUCCESS
    I->>M: 同事务切换 active_version_id + 当前版本 ACTIVE
    opt 旧活动版本存在
        I->>M: 同事务把旧版本置为 SUPERSEDED
        I->>X: 同事务写 CLEAN_REQUESTED
    end
```

1. API 接收文件流，边上传到 OSS staging key，边计算 `file_hash`
2. OSS staging 上传成功后，在第一个事务中完成：
   - 查找/创建 `documents`
   - 做去重判断
   - 创建 `document_versions(storage_status=PENDING_PROMOTE)`
3. 如果第一个事务失败，立即对 staging object 做 best-effort 回收
4. 第一个事务提交后，由应用服务把 staging object promote/copy 到正式 `storage_key`，并校验对象可读性、大小和哈希
5. promote 成功后，在第二个事务中：
   - 更新 `storage_status=READY`
   - 清空 `staging_storage_key`
   - 写入 `outbox_events(PARSE_REQUESTED)`
6. 如果 promote 失败，版本标记 `storage_status=FAILED`，记录错误并由人工或后台重试；此时不能提前投递解析任务
7. 第二个事务提交后由业务层 best-effort 触发一次 Dispatcher
8. Celery Beat 每 5 秒兜底扫描一次 Outbox
9. Dispatcher 读取 Outbox 并投递解析任务
10. Parser 拉取文件、解析、切片，在一个事务中：
   - 覆盖写入该 `version_id` 的切片
   - 更新 `parse_status=SUCCESS`
   - 初始化或更新 `projection_sync_status`
   - 写入 `outbox_events(INDEX_REQUESTED)`
11. Dispatcher 再次读取 Outbox，并投递索引任务
12. Indexer 读取切片，并发写 Milvus / ES
13. 两边都成功后，在事务中：
   - 更新两个 `projection_sync_status=SUCCESS`
   - 更新 `index_status=SUCCESS`
   - 切换 `documents.active_version_id`
   - 当前版本 `visibility_status=ACTIVE`
...
2. `Parser` 只允许处理 `storage_status=READY` 的版本；对于 `PENDING_PROMOTE/FAILED/DELETED` 版本直接拒绝执行。
3. `Indexer` 只允许对 `parse_status=SUCCESS` 且 `index_status in (PENDING, FAILED)` 的版本执行。
4. 首版以 Celery 为统一异步任务载体；Dispatcher 的职责是从 Outbox 调用 Celery，而不是直接在事务里发任务。
5. `queue_name` 应在 Outbox 阶段就确定，例如 `parse_jobs`、`index_jobs`、`clean_jobs`、`repair_jobs`、`maintenance_jobs`。
6. 同一 `document_id` 在任意时刻只允许一个未完成的新版本处于在途状态。
...
3. 历史版本曾经出现过相同 `file_hash`，也不意味着可以直接复用旧版本记录；是否新建版本仍以当前活动版本和当前解析配置为准。
4. 首版不做跨文档对象存储复用，避免 Cleaner 删除时误伤其他文档引用。
5. `PARSE_REQUESTED` 只能在对象 `storage_status=READY` 后投递，不能把“文件已上传到 staging”误判为“版本已经可解析”。
6. 历史版本源文件保留策略要单独约定，不要默认“切换成功即硬删全部旧对象”。
...
2. Parser 只处理 `storage_status=READY` 的版本，避免对象尚未 promote 完成时抢跑
3. Parser 必须可重复执行
4. 再次执行时应覆盖同一版本切片，而不是追加
5. Parser 生成的 `chunk_uid` 必须稳定，确保重跑时投影层是 upsert 语义
6. 建议绑定到 `parse_jobs` 队列，默认使用 Celery `prefork` worker
7. 如果 `parser_config_hash` 变化，应拒绝原地增量重跑，改走整版本重建
...
4. 删除源文件前要校验对象归属和保留策略，避免误删仍需审计或回放的版本对象
5. 建议绑定到 `clean_jobs` 队列
...
## 19. 以中小型企业生产级视角仍需重点盯住的问题

即使按本规划实现，下面这些点仍然值得在代码落地前继续确认：

1. **查询侧必须严格按 `active_version_id` 或等价版本条件过滤**。如果未来问答/API 只按 `document_id` 或 `kb_id` 查询，Cleaner 稍有延迟就会混入旧版本结果。
2. **需要最小运维面板或后台接口**。仅靠数据库字段还不够，至少要能查看 `FAILED` 版本、重投任务、查看对象状态和对账结果，否则出故障只能手工改库。
3. **需要明确配额与成本保护**。中小企业环境最常见的问题不是理论上一致性，而是大文件、频繁重传、失败重试把 MySQL、OSS、队列和 Embedding 成本打满。
4. **需要补监控和告警基线**。至少要有队列积压、任务失败率、对象 promote 失败率、索引重建次数、Cleaner/Janitor 滞留量监控，否则问题只能靠用户投诉发现。
5. **需要定义人工干预边界**。例如 `storage_status=FAILED`、`index_status=FAILED`、死信任务达到上限后，谁能重试、谁能废弃、是否允许强制切换，都应在实现前定规矩。
