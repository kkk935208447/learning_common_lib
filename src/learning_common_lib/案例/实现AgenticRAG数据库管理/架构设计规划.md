...
4. `row_version` 用于 Parser / Indexer / Cleaner 在并发更新版本状态时做乐观锁控制。
...
4. 旧版本改为 `SUPERSEDED`
5. Outbox 写入旧版本 `CLEAN_REQUESTED`，异步清理旧版本投影
6. 旧版本 MySQL 记录默认保留，仅作为审计与排障依据，不再参与在线检索
...
5. 每个模块在各自子目录中维护自己的抽象基类，不额外抽一个全局 `base` 层统一管理。
