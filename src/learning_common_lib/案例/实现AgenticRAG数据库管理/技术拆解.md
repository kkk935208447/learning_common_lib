...
2. 文件上传成功后，再进入 MySQL 事务。
3. 事务里写 `documents`、`document_versions`、`outbox_events`。
4. 如果事务失败，对 staging object 做 best-effort 回收。

这样做的原因：

1. 关系数据库事务无法覆盖 OSS 写入。
2. 把外部 I/O 放在事务外，能减少持锁时间。
3. 事务只负责固化业务状态，不负责承接大文件传输。
...
4. 如果 `file_hash` 不同，则新建 `document_versions(version_no + 1)`，初始状态为 `STAGED`。
5. 新版本完成解析和索引后，才切换 `documents.active_version_id`。
6. 切换完成后，旧版本只保留 MySQL 记录，异步清理 Milvus / ES 投影。
...
sequenceDiagram
    ...
    API->>DB: 创建新版本(STAGED)
    PARSE->>DB: 写 chunks + parse_status=SUCCESS
    INDEX->>DB: 读取 chunks
    INDEX->>INDEX: 双写 Milvus / ES
    INDEX->>DB: 事务切换 active_version_id
    INDEX->>DB: 旧版本标记 SUPERSEDED
    INDEX->>CLEAN: 投递旧版本清理任务
```
...
2. 把活动版本和历史未删版本打上 `DELETE_PENDING`。
...
3. OSS 原文件是否保留取决于审计要求；首版可默认保留最近活动版本的源文件，旧版本文件按策略清理。
...
1. 存储模块维护自己的 `ports/storage/base.py`
2. 向量模块维护自己的 `ports/vector_store/base.py`
3. 检索模块维护自己的 `ports/search_store/base.py`
4. 队列模块维护自己的 `ports/queue/base.py`
5. 锁模块维护自己的 `ports/locks/base.py`
