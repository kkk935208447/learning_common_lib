"""
解决什么问题: 提供 Celery 任务场景的异常层级树，区分可重试/不可重试异常，统一重试决策
...
与 Celery 内置异常的映射:
  celery.exceptions.Retry           → 框架自动重试信号（task.retry() 触发）
  celery.exceptions.MaxRetriesExceededError → 重试次数耗尽
  celery.exceptions.SoftTimeLimitExceeded   → 对应 TaskTimeoutError
  billiard.exceptions.WorkerLostError       → 对应 task_reject_on_worker_lost 配置
"""
...
class TaskTimeoutError(TaskRetryableError):
    """任务执行超时。对应 Celery SoftTimeLimitExceeded。"""
    pass
...
def _demo() -> None:
    ...
    errors: list[tuple[str, Exception]] = [
        ...
        ("LockAcquireError", LockAcquireError(
            "获取锁失败", detail={"lock_name": "order:12345", "timeout": 10}
        )),
    ]
    ...
    print("🎯 === 层级捕获 ===")
    try:
        raise LockAcquireError("获取锁失败: order:999")
    except TaskRetryableError as exc:
        print(f"  TaskRetryableError 层捕获 LockAcquireError: {exc}")
