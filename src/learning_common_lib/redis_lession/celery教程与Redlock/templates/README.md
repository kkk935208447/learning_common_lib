...
将 `templates/` 目录复制到你的项目中，通过环境变量 `CELERY_BROKER_URL` / `CELERY_RESULT_BACKEND` / `CELERY_CUSTOM_WORKER_POOL` 配置 Redis 连接与 worker pool。

锁模板现在分为两条路径：

- 纯异步路径: `distributed_lock_aio.py`，基于 `redis.asyncio` + `asyncio` 看门狗，适合 FastAPI / `async def task`
- 同步兼容路径: `distributed_lock.py`，基于 `python-redis-lock`，适合保留同步 Redis 客户端的项目

推荐优先使用 `async_distributed_lock()` / `AsyncRedisWatchdogLock`；只有同步调用侧才使用 `distributed_lock()`。装饰器语法糖只在重复样板很多时再启用。每个模板文件底部都有 `_demo()` 函数，可直接运行查看效果。

## 同步边界说明

这套模板是 async-first，但并不是“所有底层客户端都已经 fully async”：
...
- 只有 `distributed_lock_aio.py` 这条锁路径是原生 async；`distributed_lock.py` 仍然是同步 Redis 客户端 + `python-redis-lock`

因此，这里的准确说法应是：

- `async-friendly` 调用方式已经就位
- worker 侧 `async def task` 已经跑在 asyncio 上
- 但发布、查结果这些边界仍然是“同步客户端 + 线程包装”
- 锁能力则同时提供“纯异步实现”和“同步兼容实现”两条正式路径
...
- **core 层**：`celery_config`, `celery_app`, `error_handling`, `task_base`, `distributed_lock`, `distributed_lock_aio`
- **集成层**：`fastapi_celery`
...
```python
from templates import with_async_lock, with_lock
```
...
| `distributed_lock_aio.py` | 纯异步 Redis 看门狗锁，适合 FastAPI / `async def task` |
...
| Celery 长任务锁保护（纯 async） | `distributed_lock_aio` | 默认推荐 `async_distributed_lock()` / `AsyncRedisWatchdogLock` |
| Celery 长任务锁保护（同步兼容） | `distributed_lock` | 仅在项目仍使用同步 Redis 客户端时采用 |
...
5. `distributed_lock_aio.py` — 纯异步 Redis 看门狗锁
6. `distributed_lock.py` — 同步 Redis / `python-redis-lock` 兼容路径
7. `fastapi_celery.py` — FastAPI 集成全貌
...
- [ ] 纯异步项目优先使用 `distributed_lock_aio.py`
- [ ] Celery 长任务优先开启 `auto_renewal=True`
- [ ] 如需同步兼容路径，再使用 `python-redis-lock` + `distributed_lock.py`
