"""
解决什么问题: 提供生产级中间件栈，包含结构化日志、指数退避重试、慢任务告警
输入输出约定: create_default_middlewares() 返回 list[TaskiqMiddleware]，按推荐顺序排列
失败策略: LoggingMiddleware 仅记录不干预；RetryMiddleware 根据 labels 决定重试；SlowTaskWarningMiddleware 仅记录慢任务告警，不取消任务
...
中间件栈（推荐顺序）:
  ...
  3. SlowTaskWarningMiddleware — 慢任务告警
"""
...
# ---------------------------------------------------------------------------
# SlowTaskWarningMiddleware — 慢任务告警
# ---------------------------------------------------------------------------


class SlowTaskWarningMiddleware(TaskiqMiddleware):
    """慢任务告警中间件。

    从 message.labels 读取 warn_after_s（默认 300 秒）。
    旧的 timeout 标签也会被当作阈值读取，便于和教程中的基础示例共存。
    它只负责记录告警日志，不会取消任务，也不会替代真正的超时控制。
    """
    ...
    async def post_execute(self, message: TaskiqMessage, result: TaskiqResult) -> None:
        ...
        if execution_time > warn_after_s:
            logger.warning(
                "慢任务告警 | task_id=%s | task_name=%s | execution_time=%.3fs | warn_after_s=%.1fs",
                message.task_id,
                message.task_name,
                execution_time,
                warn_after_s,
...
def _demo() -> None:
    ...
    example_labels: dict[str, Any] = {
        ...
        "warn_after_s": "60",     # 执行超过 60 秒就告警（默认 300）
    }
    ...
    print()
    print("  # 任务级别配置重试:")
    print('  @broker.task(max_retries="5", retry_delay="2.0", warn_after_s="60")')
