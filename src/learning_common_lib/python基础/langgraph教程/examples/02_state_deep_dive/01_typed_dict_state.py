...
目标：理解 TypedDict 作为 LangGraph 状态的基本用法
关键 API：StateGraph, TypedDict, total 参数
运行命令：python 01_typed_dict_state.py
预期现象：演示 total=True（默认）与 total=False 的区别，以及 last-write-wins 覆盖行为
生产提醒：TypedDict 仅提供类型提示，运行时不做校验；需要运行时校验请用 Pydantic
"""
